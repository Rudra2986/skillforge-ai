...
export default function ReadinessScoreCard({ 
  ...
  const milestoneProgressPercent = Math.round((completedMilestones / totalMilestones) * 100);

  const skillsCount = skills.length || 0;
  const skillsTarget = 8;
  const skillsProgressPercent = Math.min(100, Math.round((skillsCount / skillsTarget) * 100));

  const projectsCount = projects.length || 0;
  const projectsTarget = 3;
  const projectsProgressPercent = Math.min(100, Math.round((projectsCount / projectsTarget) * 100));
    ...
    <div className="rounded-xl bg-canvas-subtle border border-border p-5 space-y-5 shadow-sm text-left font-sans">
      ...
      <div className="p-4 rounded-lg bg-canvas-surface border border-border-subtle space-y-3">
        ...
        <div className="space-y-1.5 pt-1">
          <div className="w-full h-2 bg-canvas rounded-full overflow-hidden flex p-0.5 border border-border-subtle gap-0.5">
            <div 
              className="h-full bg-blue-500 rounded-l-full transition-all duration-500"
              style={{ width: `${Math.round((skillsProgressPercent * 0.40))}%` }}
              title={`Technical Skills: ${skillsProgressPercent}%`}
            />
            <div 
              className="h-full bg-indigo-500 transition-all duration-500"
              style={{ width: `${Math.round((milestoneProgressPercent * 0.35))}%` }}
              title={`Roadmap Execution: ${milestoneProgressPercent}%`}
            />
            <div 
              className="h-full bg-emerald-500 rounded-r-full transition-all duration-500"
              style={{ width: `${Math.round((projectsProgressPercent * 0.25))}%` }}
              title={`Portfolio Proof: ${projectsProgressPercent}%`}
            />
          </div>
      ...
      <div className="space-y-2">
        {/* Row 1: Skills */}
        <div className="p-2.5 rounded-lg bg-canvas-surface/70 border border-border-subtle flex items-center justify-between text-xs hover:border-border interactive-transition">
