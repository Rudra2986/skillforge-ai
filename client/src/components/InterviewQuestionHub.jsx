...
export default function InterviewQuestionHub({ questions = [], targetRole = "Full-Stack AI Engineer" }) {
  ...
  const handleEvaluate = async (q) => {
    ...
    try {
      // Call Gemini AI Evaluate Endpoint
          ...
          return (
            <div
              ...
              className={`rounded-xl border transition-all duration-200 overflow-hidden ${
                isExpanded 
                  ? 'border-accent bg-canvas-surface/80 shadow-md' 
            ...
            >
              ...
              <button
                ...
                className="w-full p-4 sm:p-5 flex items-start justify-between gap-3 text-left interactive-transition cursor-pointer"
              >
                <div className="space-y-2 flex-1">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="px-2 py-0.5 rounded text-[10px] font-mono uppercase bg-accent/20 border border-accent/40 text-accent-text font-semibold">
                      {q.topic || targetRole}
                    </span>
                    <span className="px-2 py-0.5 rounded text-[10px] font-mono bg-canvas-surface border border-border-subtle text-neutral-400">
                      High Frequency Technical Question
                    </span>
                    {evaluation && (
